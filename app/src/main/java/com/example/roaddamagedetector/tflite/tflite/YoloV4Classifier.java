...
package com.example.roaddamagedetector.tflite.tflite;
...
public class YoloV4Classifier implements Classifier {
    ...
    private ArrayList<Recognition> getDetectionsForTiny(ByteBuffer byteBuffer, Bitmap bitmap) {
        ...
        for (int i = 0; i < gridWidth;i++){
            ...
            for (int c = 0;c< labels.size();c++){
                classes [c] = out_score[0][i][c];
                Log.d("Locationclasses", Float.toString(classes[c]) );
            }
            for (int c = 0;c<labels.size();++c){
                if (classes[c] > maxClass){
                    detectedClass = c;
            ...
            final float score = maxClass;
            Log.d("Locationscore", Float.toString(score));
            if (score > getObjThresh()){
                final float xPos = bboxes[0][i][0];
                final float yPos = bboxes[0][i][1];
                final float w = bboxes[0][i][2];
                final float h = bboxes[0][i][3];
                Log.d("Location", Float.toString(xPos));
                Log.d("Location", Float.toString(w));
                final float x = xPos - w / 2;
                Log.d("Location", Float.toString(x));
                Log.d("Location", Float.toString(i));
                Log.d("Location", Float.toString(gridWidth));
                final RectF rectF = new RectF(
                        Math.max(0, xPos - w / 2),
                        Math.max(0, yPos - h / 2),
                        Math.min(bitmap.getWidth() - 1, xPos + w / 2),
